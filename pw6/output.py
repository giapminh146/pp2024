def select_courses(self, stdscr):
    ...
    for i, course in enumerate(self.get_courses(), 1):
        stdscr.addstr(f"{i}. {course.get_course_name()} - {course.get_course_id()} - ({course.get_credits()} credits)\n")
    ...
    while True:
        ...
        try:
            ...
            if choice == 0:
                ...
                stdscr.addstr(f"Selected course: {selected_course.get_course_name()}\n")
                break
            else:
                stdscr.addstr(
...
def list_students(self, stdscr):
    ...
    if len(self.get_students()) == 0:
        ...
        for i, student in enumerate(self.get_students(), 1):
            stdscr.addstr(f"{i}. {student.get_student_name()} - {student.get_student_id()} - DoB: {student.get_student_dob()}\n")
    stdscr.refresh()
...
def show_marks(self, stdscr):
    ...
    stdscr.addstr(f"Marks for {selected_course.get_course_name()}:\n")
    for i, student in enumerate(self.get_students(), 1):
