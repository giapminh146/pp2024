...
def save_students_info(student_information_list):
    try:
        ...
        for student_info in student_information_list:
            f.write(f"{student_info['student_id']} - {student_info['student_name']} - {student_info['student_DoB']}\n")
        f.close()
    except IOError:
        print(f"Error writing to {'students.txt'}: {IOError}")


def save_courses_info(course_information_list):
    try:
        with open('courses.txt', 'a') as f:
            for course_info in course_information_list:
                f.write(f"{course_info['course_id']} - {course_info['course_name']} - {course_info['credits']}\n")
    except IOError:
        print(f"Error writing to {'courses.txt'}: {IOError}")
        ...
        with open('marks.txt', 'a') as f:
            for student in self.get_students():
                ...
                if selected_course.get_course_name() in student.get_marks():
                    f.write(f"{selected_course.get_course_name()} - {student_name} ({student_id}) - {student.get_marks()[selected_course.get_course_name()]}\n")
    except IOError:
        print(f"Error writing to {'marks.txt'}: {IOError}")
